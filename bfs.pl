...
% containing StartState, a variable for the EndNode, and the GoalState. When bfs/4 succeeds, 
% bfs/2 prints the path from the StartState to the GoalState.
% A node contains a state, the action that produced the state, and node's parent node.
%

bfs(StartState,GoalState) :-
  bfs([node(StartState,start,nil)],[StartState],EndNode,GoalState),
  printPath(EndNode).
...
bfs(Fringe,_Explored,node(search_failed,nil),_GoalState) :-
  isEmptyQueue(Fringe).
...
bfs(Fringe,_Explored,Node,GoalState) :-
  peekQueue(Node,Fringe),
  isGoalState(Node,GoalState).

%
% Otherwise, expand the oldest node in the fringe and continue the search.
%
bfs(Fringe,Explored,EndNode,GoalState) :-
  dequeue(Node,Fringe,Fringe2),
  expand(Node,Explored,NewExplored,Fringe2,NewFringe),
  bfs(NewFringe,NewExplored,EndNode,GoalState).
