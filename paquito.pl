...
in(paquito,frontEntrance).

is_empty([]):- true.
actsP([]).

paquito([run]):-start,run.

paquito(Agent2,Acts2,[greet(paquito,Agent2)]):-
	is_empty(Acts2),
	retract(actsP(_)),assertz(actsP([greet(paquito,Agent2)])),!.

paquito(Agent2,Acts2,[(greet(paquito,Agent2))]):-
	contains(greet(Agent2,paquito),Acts2),
	actsP(Acts1),
	\+contains(greet(paquito,Agent2),Acts1),
	retract(actsP(_)),assertz(actsP([greet(paquito,Agent2)])),!.

paquito(Agent2,Acts2,[request(paquito,Agent2,move(Room))]):-
	actsP(Acts1),
	contains(greet(paquito,Agent2),Acts1),
	((contains(greet(Agent2,paquito),Acts2)) ;(contains(agree(request(paquito,Agent2,move(_))),Acts2))),
	in(paquito,R),
	\+R=win,
	move2,
	in(paquito,Room),
	retract(actsP(_)),assertz(actsP([request(paquito,Agent2,move(Room))|Acts1])),!.

paquito(Agent2,Acts2,[agree(request(Agent2,paquito,move(Room)))]):-
	actsP(Acts1),
	contains(greet(paquito,Agent2),Acts1),
	contains(request(Agent2,paquito,move(Room)),Acts2),
	movepaquito(Room),
	in(paquito,Room),
	retract(actsP(_)),assertz(actsP([agree(request(Agent2,paquito,move(Room)))|Acts1])),!.

paquito(Agent2,Acts2,[inform(Agent2,paquito,'we escaped')]):-
	actsP(Acts1),
	in(paquito,win),
	in(Agent2,win),
	retract(game(_)),assertz(game(done)),
	retract(actsP(_)),assertz(actsP([inform(Agent2,paquito,'we escaped')|Acts1])),!.

%------------------------------------------------------------------------------

start:-
format("Start game!~n~n"),
	retractall(in(paquito,_)), assert(in(paquito,frontEntrance)),
...
movepaquito(Room):-
	retract((in(paquito,_))),
	assertz((in(paquito,Room))).
...
%gets room in map that needs an item that paquito has
getItemRoom([],nope).
...
move2:-
	...
	\+RM=nope,
	addRoom(RM),
	retract(in(paquito,_)),assertz(in(paquito,RM)),
	get_items,
	use_items,
	checkAgents(RM),
	print_description,
...
go_back([]). 
go_back(R,[R|T]):- retractall(in(paquito,_)),assert(in(paquito,R)),addRoom(R),use_items.
go_back(Room,[H|T]):- retractall(in(paquito,_)),assert(in(paquito,H)),addRoom(H),use_items, go_back(Room,T).
